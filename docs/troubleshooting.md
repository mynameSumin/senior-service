...
## 2. 심평원 요양병원 평가등급(HIRA) — khqa.kr은 막혀 있지만 hira.or.kr 자체 페이지는 열려 있었다

처음엔 요양병원 적정성평가의 게시 위치를 `khqa.kr`(병원평가통합포털)로 알고 있었는데, 이 포털의
robots.txt는 `Disallow: /`(루트만 허용)로 전체 차단이라 포기했었다.

그런데 같은 평가 데이터를 `hira.or.kr` 메인 사이트의 "병원·약국찾기 > 우리지역 좋은병원 찾기"
화면도 보여주고 있었고, **이 경로는 robots.txt에서 `/co/`만 막혀 있어 차단 대상이 아니었다**
(`/ra/hosp/*`는 허용). 화면이 쓰는 JS 파일(`HospitalMap.js`)을 읽어 내부 AJAX 엔드포인트를
역추적했다:
...
서비스키 없이 페이지당 20건씩 응답하며, 전국 1,220개 요양병원의 최신 평가등급(1~5등급, 1이
최우수)을 그대로 받을 수 있었다. 기존 1~5등급을 서비스 전체에서 쓰는 A~E 스케일로 변환해
(`GRADE_1_5_TO_AE`) `evaluations` 테이블에 `source="심평원"`으로 적재했다(`crawlers/hira/run.ts`).
응답에 평가연도가 없어(최신 등급만 제공) `eval_year`는 크롤링 시점의 연도로 기록했다 — 등급
추이 비교는 이 소스만으로는 계산되지 않는다.

이 발견 전까지는 "요양병원 평가 정보는 robots.txt로 막혀 있다"고 안내했었는데, 실제로는
khqa.kr만 막혀 있었을 뿐 hira.or.kr 자체 경로는 열려 있었다 — robots.txt는 사이트 단위가 아니라
경로 단위로 정확히 확인해야 한다는 교훈.
...
| 심평원/HIRA 요양병원 | fuzzy 매칭 크롤링           | khqa.kr은 차단, 그러나 hira.or.kr 자체 AJAX는 허용 → 1,220곳 등급 확보    |
...
## 10. 요양병원 크롤러가 동명의 요양원에 평가를 잘못 붙였다

`crawl:hira`를 처음 돌렸을 때 "신규 시설 1,012곳, 평가등급 적재 1,033건"으로 끝났는데, 1,220건
중 208건이 "기존 시설과 매칭"되어 새로 만들어지지 않았다. 이상해서 확인해보니 매칭 대상 풀이
`facilities` 전체(요양원/주야간보호/방문요양 포함 25,262곳)였던 게 문제였다 — **요양병원은
건강보험(의료기관) 영역이고 요양원 등은 장기요양보험(요양기관) 영역이라 같은 시설일 수 없는데**,
이름이 비슷한 요양원(예: "OO요양병원" vs "OO요양원")에 `bestFacilityMatch`가 0.6 이상의
신뢰도로 오매칭한 것이었다. 검증해보니 정확히 26건의 평가가 `type="요양원"`인 시설에 잘못
붙어 있었다.

같은 기관 종류가 아니면 애초에 매칭 후보가 될 수 없으므로, 매칭 풀을 기존 `type="요양병원"`
시설로만 제한하도록 고쳤다(`crawlers/hira/run.ts`) — 재실행 시 중복 생성만 막고, 다른 종류의
시설과는 절대 매칭되지 않는다. 오매칭된 26건의 evaluations를 지우고 크롤러를 재실행해 해당
26곳을 신규 요양병원 시설로 올바르게 적재했다. 일반화하면: **fuzzy 매칭은 후보 풀을 "같은
종류"로 먼저 좁힌 뒤에 돌려야 한다** — 전체 풀 대상 매칭은 후보가 늘어날수록 동명 오매칭
위험이 커진다(8번 항목의 운영자 변경 케이스와 같은 종류의 문제).

## 11. 요양병원 비급여 비용 — 다른 제공기관 키라 처음엔 Forbidden, 원인은 http/https

요양원 등은 국민건강보험공단 API(제공기관 B550928)로 비급여 비용을 실시간 조회하고 있었는데,
요양병원은 장기요양보험 영역 밖이라 이 API에 데이터가 없다(2번 항목 참고). 같은 평가 데이터를
찾았던 것과 같은 방식으로 data.go.kr에서 "건강보험심사평가원_비급여진료비정보조회서비스"
(제공기관 B551182)를 찾았는데, 기존 `DATA_GO_KR_SERVICE_KEY`로 호출하면 `Forbidden`이 떴다.

data.go.kr Open API는 **계정 단위가 아니라 제공기관(API)별로 개별 활용신청**이 필요하다 —
B550928용으로 승인된 키라고 B551182도 자동으로 열리지 않는다. 사용자가 B551182를 별도로
활용신청·승인받은 뒤 같은 키로 다시 호출했는데도 여전히 `Forbidden`이 떴다. `curl -i`로 응답
헤더를 보니 본문이 XML 에러 응답이 아니라 순수 텍스트 `Forbidden`(10바이트)이었던 게 단서였다
— data.go.kr 게이트웨이는 `http://`로 들어오는 이 API 요청을 그냥 막아버리고, **`https://`로
호출하니 정상 XML 응답**이 돌아왔다(기존 B550928 API는 `http://`로도 잘 동작해서 의심하지
못했다). 같은 게이트웨이라도 제공기관마다 프로토콜 강제 여부가 다를 수 있다는 뜻.
